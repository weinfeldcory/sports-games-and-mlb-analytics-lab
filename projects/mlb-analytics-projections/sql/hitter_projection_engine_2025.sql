CREATE OR REPLACE TABLE hitter_projection_engine_2025 AS
WITH training AS (
    SELECT
        "IDfg" AS player_id,
        "Name" AS player_name,
        ...
        END AS pos_per_pa,
        CASE
            WHEN "Season" = 2024 THEN 0.65
            WHEN "Season" = 2023 THEN 0.35
            ELSE 0.0
...
player_history AS (
    SELECT
        ...
        MAX_BY(age, season) + 1 AS target_age_2025,
...
player_priors AS (
    SELECT
        ph.*,
        CASE
            WHEN target_age_2025 <= 24 THEN '24_and_under'
            WHEN target_age_2025 BETWEEN 25 AND 27 THEN '25_27'
            WHEN target_age_2025 BETWEEN 28 AND 30 THEN '28_30'
            WHEN target_age_2025 BETWEEN 31 AND 33 THEN '31_33'
            ELSE '34_plus'
        END AS age_bucket,
        CASE
    ...
    SELECT
        *,
        GREATEST(
            0.78,
            1.0
            - 0.012 * GREATEST(target_age_2025 - 31, 0)
            - 0.006 * GREATEST(23 - target_age_2025, 0)
        ) AS durability_age_factor,
        GREATEST(
            0.85,
            1.0
            - 0.010 * GREATEST(target_age_2025 - 30, 0)
            - 0.004 * GREATEST(26 - target_age_2025, 0)
        ) AS power_age_factor,
        GREATEST(
            0.88,
            1.0
            - 0.006 * GREATEST(target_age_2025 - 31, 0)
            - 0.003 * GREATEST(24 - target_age_2025, 0)
        ) AS contact_age_factor,
        GREATEST(
            0.70,
            1.0
            - 0.025 * GREATEST(target_age_2025 - 26, 0)
            - 0.008 * GREATEST(23 - target_age_2025, 0)
        ) AS speed_age_factor,
        GREATEST(
            0.72,
            1.0
            - 0.018 * GREATEST(target_age_2025 - 28, 0)
            - 0.006 * GREATEST(24 - target_age_2025, 0)
...
projected_components AS (
    SELECT
        ...
        player_name,
        team_2024,
        target_age_2025 AS projected_age,
        weighted_pa_sample,
        LEAST(
            1.20,
            1.0
...
actual_2025 AS (
    SELECT
        "IDfg" AS player_id,
        "Team" AS actual_team_2025,
        "PA" AS actual_pa_2025,
        "G" AS actual_games_2025,
        "WAR" AS actual_war_2025,
        "wRC+" AS actual_wrc_plus_2025,
        "wOBA" AS actual_woba_2025,
        "xwOBA" AS actual_xwoba_2025,
        "BB%" AS actual_bb_rate_2025,
        "K%" AS actual_k_rate_2025,
        "HR" AS actual_home_runs_2025,
        "SB" AS actual_stolen_bases_2025,
        "Def" AS actual_defense_runs_2025
    FROM batting_stats
    WHERE "Season" = 2025
),
scored AS (
    SELECT
        ...
        a.actual_pa_2025,
        a.actual_war_2025,
        a.actual_wrc_plus_2025,
        a.actual_woba_2025,
        a.actual_xwoba_2025,
        a.actual_bb_rate_2025,
        a.actual_k_rate_2025,
        a.actual_home_runs_2025,
        a.actual_stolen_bases_2025,
        a.actual_defense_runs_2025
    FROM final_projection AS fp
    CROSS JOIN league_context AS lc
    LEFT JOIN actual_2025 AS a
        ON fp.player_id = a.player_id
)
SELECT
    *,
    projected_pa - actual_pa_2025 AS pa_error,
    projected_home_runs - actual_home_runs_2025 AS hr_error,
    projected_stolen_bases - actual_stolen_bases_2025 AS sb_error,
    projected_woba - actual_woba_2025 AS woba_error,
    projected_value_war_proxy - actual_war_2025 AS value_war_proxy_error
FROM scored;
